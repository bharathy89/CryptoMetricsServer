...
def operation_eval(operation: Operation, sample_interval):
    ...
    result1 = operand_eval(
        operation.operand_type1,
    ...
    if operation.operand_type2:
        result2 = operand_eval(
            operation.operand_type2,
...
def perform_operator(result1, result2, operator):
    if isinstance(result1, list) and isinstance(result2, list):
        ...
        if len(result1) == len(result2):
            ...
            logger.info(
                "wrong sampling interval between operands "
                "result1: {r1} result2: {r2}".format(r1=result1, r2=result2)
            )
            raise Exception(
                "wrong sampling interval between "
                "operands result1: {r1} result2: {r2}".format(
                    r1=len(result1), r2=len(result2)
                )
            )
...
def perform_conditional_operator(result1, result2, operator):
    ...
    if (
        isinstance(result1, list)
        and isinstance(result2, list)
...
def operand_eval(
    ...
    if operand_type == "operation" and isinstance(operand_value, Operation):
        ...
        raise ValueError(
            "operand is invalid format: " + operand_type + " : " + str(operand_value)
        )
...
def monitor_eval(monitor: Monitor):
    if monitor.condition:
        triggered = False
        notified_time = None
        result1 = operation_eval(monitor.condition.operand_1, monitor.sample_interval)
        result2 = operation_eval(monitor.condition.operand_2, monitor.sample_interval)
